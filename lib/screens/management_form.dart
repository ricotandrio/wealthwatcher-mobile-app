...
class ManagementForm extends StatelessWidget {
  final typeManagement;

  ManagementForm({required this.typeManagement});
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    typeManagement == Strings.expense
                        ...
                        : BlocConsumer<AddIncomeBloc, AddIncomeState>(
                            listener: (context, state) {
                              if (state is AuthenticatedAddIncome) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(Strings.addIncomeFailed),
                                  ),
                                );
                              } else if (state is LoadingAddIncome) {
                                showDialog(
                                  context: context,
                                  barrierDismissible: false,
                                  builder: (BuildContext context) {
                            ...
                            builder: (context, state) {
                              return ElevatedButton(
                                onPressed: () {
                                  if (_formKey.currentState!.validate()) {
                                    BlocProvider.of<AddIncomeBloc>(context)
                                        .add(AddIncome(
                                      category: typeCategory,
                                      name: _name.text,
                                      amount: double.parse(_amount.text),
